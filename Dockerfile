...
RUN echo "Generating spark-2.0.2-bin-hadoop2.7.tgz file" && \
    cd spark-2.0.2-bin-hadoop2.7 && \
    cat x* > spark-2.0.2-bin-hadoop2.7.tar.gz && \
    rm -rf x* && \
    tar -xzf spark-2.0.2-bin-hadoop2.7.tar.gz && \
    rm -rf spark-2.0.2-bin-hadoop2.7.tar.gz && \
    mv spark-2.0.2-bin-hadoop2.7 /usr/local/spark && \
    cd .. && rm -rf spark-2.0.2-bin-hadoop2.7 && \
